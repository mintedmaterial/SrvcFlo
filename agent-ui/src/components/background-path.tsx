...
function useLogoRain() {
  ...
  useEffect(() => {
    ...
    // Create fallback images client-side only
    const fallbackConfigs = [
      { color: '#3B82F6', text: 'S' },  // Sonic blue
      { color: '#8B5CF6', text: 'D' },  // DeFi purple
      { color: '#10B981', text: 'N' },  // NFT green
      { color: '#F59E0B', text: 'T' },  // Token orange
    ]
    const clientFallbackImages = fallbackConfigs.map(config =>
      createFallbackImage(config.color, config.text)
    ).filter(Boolean) as HTMLImageElement[]

    // Add fallback images first (always available)
    clientFallbackImages.forEach((fallbackImg, i) => {
      imgs.push(fallbackImg)
      loadedImagesRef.current.add(imgs.length - 1)
      loadedCount++
    })

    console.log(`🎨 Added ${clientFallbackImages.length} fallback images`)
    ...
    logoPngs.forEach((src, i) => {
      ...
      img.onload = () => {
        ...
        loadedCount++
        console.log(`✅ Loaded image ${loadedCount}/${logoPngs.length + fallbackConfigs.length}: ${src.split('/').pop()}`)
        
        // Start rain after fallbacks + a few external images load
        if (loadedCount >= clientFallbackImages.length + 2 && !rainStartTimeRef.current) {
    ...
    setTimeout(() => {
      if (!rainStartTimeRef.current && loadedImagesRef.current.size >= clientFallbackImages.length) {
        rainStartTimeRef.current = Date.now()
        initializeDrops()
