...
def audit_contract_with_chaingpt(contract_code: str, contract_name: str = "Contract") -> Dict[str, Any]:
    ...
    try:
        ...
        if response.status_code == 200:
            # ChainGPT streaming response - may be JSON or text
            try:
                # Try to parse as JSON first
                result = response.json()
                # ChainGPT might return different structures
                if isinstance(result, dict):
                    audit_result = result.get("response", result.get("data", result.get("content", response.text)))
                else:
            ...
            return {
                ...
                "raw_response": response.text[:500]  # Include raw response for debugging
            }
        else:
            return {
