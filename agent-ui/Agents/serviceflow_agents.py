...
# GoogleSearchTools removed due to dependency issues
# Newspaper4kTools removed due to dependency issues
...
# Workflow import removed - not needed for basic agents
...
srvcflo_agent = Agent(
    ...
    tools=[PythonTools(base_dir=tmp_dir), DiscordTools(), XTools()],
    markdown=True,
)

# Import sonic ecosystem tools
from ecosystem_analyst_agent import get_sonic_price_data, get_sonic_market_sentiment, search_sonic_defi_opportunities
...
viral_researcher = Agent(
    ...
    """),
    instructions=dedent("""\
    You are SrvcFlo's enhanced research specialist with access to real-time Sonic ecosystem data, 
    tasked with finding the most compelling, shareable data and stories for ServiceFlow AI content.

    🔍 ENHANCED RESEARCH CAPABILITIES:
    1. VIRAL STATISTICS & MARKET DATA
       ...
       - **NEW: Real-time Sonic ecosystem price and DeFi data**
       - **NEW: Crypto market sentiment for blockchain integration content**

    2. EMOTIONAL TRIGGERS & SUCCESS STORIES
       ...
       - **NEW: Crypto adoption success stories in traditional businesses**

    3. TRENDING TOPICS & OPPORTUNITIES
       ...
       - **NEW: Sonic Labs DeFi opportunities for business payment systems**
       - **NEW: Blockchain integration trends in service industries**

    4. SONIC ECOSYSTEM INTELLIGENCE (NEW)
       - Monitor S-USD token performance for payment integration content
       - Track DeFi yield opportunities relevant to business treasury management
       - Analyze market sentiment for crypto-friendly business positioning
       - Research Sonic Labs ecosystem growth for partnership opportunities

    🎯 CONTENT GOLDMINES TO FIND:
    - HackerNews discussions about AI and automation tools
    ...
    - DuckDuckGo searches for latest automation trends
    - Comprehensive web research for market insights
    ...
    - Tech startup stories from HackerNews about business automation
...
serviceflow_content_team = Team(
    ...
    1. RESEARCH PHASE (Viral Research Specialist)
       ...
       - **NEW: Analyze Sonic ecosystem market data for blockchain integration angles**
       - **NEW: Monitor DeFi opportunities relevant to service business payments**
    ...
    2. CREATION PHASE (Viral Content Creator)
       ...
       - **NEW: Integrate crypto/blockchain benefits for forward-thinking businesses**
    ...
    3. AMPLIFICATION PHASE (Social Media Specialist)
       ...
       - **NEW: Leverage crypto community engagement for broader reach**
    ...
    4. PUBLISHING PHASE (Blog Publisher)
       ...
       - **NEW: Include blockchain integration success metrics**
...
# --- Blog Posting Tool for SrvcFlo ---
def create_blog_posting_tool(api_key: str, base_url: str = "https://srvcflo.com"):
    """Create a tool for agents to post blogs to ServiceFlow AI website"""
    ...
    def post_blog(agent, title: str, content: str, category: str = "AI Automation", 
        ...
        # Try to post to live site first
        try:
            ...
            # Fallback: Save locally for manual posting
            ...
            return f"⚠️ Could not post to live site ({str(e)}). Blog post '{title}' saved locally as {filename} for manual upload."
    
    return post_blog

# Add blog posting tool to SrvcFlo
blog_posting_tool = create_blog_posting_tool(os.getenv('SERVICEFLOW_API_KEY', 'test_key'))
srvcflo_agent.tools.append(blog_posting_tool)
...
# Demo disabled for imports - uncomment to run standalone demo
