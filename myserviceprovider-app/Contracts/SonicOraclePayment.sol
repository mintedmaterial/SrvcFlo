...
/// @title SonicOraclePayment
/// @dev Oracle-enabled payment system for ServiceFlow AI Credits
/// @notice Supports dynamic native S token pricing via Beefy Oracle
contract SonicOraclePayment is Pausable, Ownable, ReentrancyGuard {
    ...
    mapping(address => mapping(uint256 => uint256)) public userNFTCreditsSpent;
    
    // Credit packages
    struct CreditPackage {
        uint256 usdcPrice;        // Price in USDC (6 decimals)
        uint256 usdValueForS;     // USD value for native S payment (6 decimals)
    ...
    function _distributeUSDCPayment(uint256 amount) internal {
        ...
        USDC.transfer(devWallet, toLeaderboard); // Leaderboard to dev for now
    }
    ...
    function _distributeSonicPayment(uint256 amount) internal {
        ...
        payable(devWallet).transfer(toLeaderboard); // Leaderboard to dev for now
    }
    ...
    // Get total credits (ERC-20 + available NFT credits)
