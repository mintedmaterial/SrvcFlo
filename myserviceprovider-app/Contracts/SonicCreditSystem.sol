...
/**
 * @title SonicCreditSystem
 * @dev Credit-based payment system for ServiceFlow AI generations
 * Users buy credit packages instead of paying per generation
 * Only NFT minting requires individual transactions
 */
contract SonicCreditSystem is Ownable {
    // Token contracts
    ...
    // Price Oracle for dynamic wS pricing
    ...
    // Credit packages (prices in token decimals)
    struct CreditPackage {
        uint256 usdcPrice;    // Price in USDC (6 decimals) - fixed
    ...
    // Purchase credits with wS tokens (bonus credits, dynamic pricing)
